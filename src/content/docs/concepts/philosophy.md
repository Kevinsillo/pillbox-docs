...
:::tip
If you work with an AI agent on recurring projects, the single highest-value habit is closing every session with a `prescription_summary`. Future sessions that call `pill_context` will surface these summaries first, giving the agent an immediate understanding of what was done and why.
:::

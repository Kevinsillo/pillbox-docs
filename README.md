<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="resources/pillbox-logo-dark.png">
  <img src="resources/pillbox-logo.png" alt="Pillbox" width="220">
</picture>

# pillbox-docs

***Pillbox official documentation and landing page***

Part of the [Pillbox](https://github.com/Kevinsillo/pillbox) ecosystem.

</div>

Documentation and landing page for Pillbox, built with [Astro Starlight](https://starlight.astro.build) and deployed to GitHub Pages.

## Development

```bash
pnpm install
pnpm dev
```

## Main repository

→ [pillbox](https://github.com/Kevinsillo/pillbox) — install the full ecosystem from there.
